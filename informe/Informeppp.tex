...
\begin{document}
    ...
    al poder colocarse tanto al inicio como al final de la cadena vac\'ia $A$, se generan así dos \'arboles de cadenas exactamente iguales (con las mismas cadenas), solo que estas se pueden considerar ``diferentes'' por 
...
def solve_dp(S,T):
    ...
    contruyen al prefijo solo com movimientos hacia adelante.\\
...
\end{itemize}
    ...
    un ciclo que itera $m-1$ veces y en cada iteraci\'on ejecuta otro ciclo de $m-1$ iteraciones en el que a su vez se teien que en el peor caso se ejecutan 
    ...
    Comprobar si el primer caracter de $S$ est\'a en $T$ tiene un costo de $m$ y para el peor caso se ejecuta el c\'odigo dentro de la condicional, 
    ...
    de $n$ iteraciones para computar la cantidad de cadenas de prefijo $T$ donde el prefijo se obtiene solo con mivimientos hacia adelante. De modo que se obtiene: 
